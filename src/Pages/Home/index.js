...
export default function Home({ navigation }) {
    ...
    useEffect(() => {
        APICorona.get('df').then(df => {
            setCasos(df.data.casos);
            setMortesHoje(df.data.casosEstado.pop().mortes)
        ...
        <View style={styles.container}>
            <ScrollView>
                <View style={styles.conteudo}>
                    ...
                    <View>
                        <Text>Número de casos totais: {casos}</Text>
                        <Text>Mortes Hoje: {mortesHoje}</Text>
                    </View>
                    <TouchableOpacity
                        style={styles.button}
                        onPress={() => navigation.navigate('Questionario')}>
                        <Text
                            style={styles.buttonText}>Realizar teste</Text>
                    </TouchableOpacity>
                    <View>
                        <Text style={{ fontSize: wp('8%') }}>Outros testes</Text>
                        <View style={styles.testesView}>
                            <Text>
                                Questionário de ensino
                            </Text>
                            <Text>
                                300 respostas
                            </Text>
                            <TouchableOpacity style={styles.testeButton}>
                                <Text style={{ color: '#FFF', fontSize: wp('3%') }}>Participar</Text>
                            </TouchableOpacity>
                        </View>
                        <View style={styles.testesView}>
                            <Text>
                                Questionário governaça
                            </Text>
                            <Text>
                                450 respostas
                            </Text>
